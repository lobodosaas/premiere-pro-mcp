...
export function ensurePrivateBridgeDirectory(
  ...
  if (platform === "win32") {
    ...
    try {
      ...
      // can still carry inherited ACEs from a shared parent (for example a temp
      // root that grants write to a sandbox group), and those inherited grants
      // are exactly what the check is meant to remove before we trust the path.
      acl = inspectWindowsAcl(dir, true);
    } catch (error) {
      throw new Error(
